...
public class Annotate {
    ...
    public final List<MorphoResult> annotatePOSFromTokens(String[] tokens) {
        List<String> posTags = this.posTagger.posAnnotate(tokens);
        String[] posTagsArray = new String[posTags.size()];
        posTagsArray = posTags.toArray(posTagsArray);
        List<Morpheme> morphemes = this.lemmatizer.getMorphemes(tokens, posTagsArray);
        List<MorphoResult> results = new ArrayList<>();
        for (int i = 0; i < morphemes.size(); i++) {
            Morpheme morpheme = morphemes.get(i);

            if (this.dictag) {
                final String dictPosTag = this.dictMorphoTagger.tag(morpheme.getWord(), morpheme.getTag());
                morpheme.setTag(dictPosTag);
            }

            final String posId = Resources.getKafTagSet(morpheme.getTag(), lang);
            final String type = Resources.setTermType(posId);

            // dictionary lemmatizer overwrites probabilistic predictions if
            // lemma is not equal to "O"
            if (this.dictLemmatizer != null) {
                final String lemma = this.dictLemmatizer.apply(morpheme
                        .getWord(), morpheme.getTag());
                if (!lemma.equalsIgnoreCase("O")) {
                    morpheme.setLemma(lemma);
                }
            }
            results.add(new MorphoResult(type,morpheme.getLemma(), posId, morpheme.getTag()));
        }
        return results;
    }
    ...
    public class MorphoResult {
        ...
        public MorphoResult(String type, String lemma, String pos, String morphofeat) {
            this.type = type;
            this.lemma = lemma;
            this.pos = pos;
